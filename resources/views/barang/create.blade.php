...
<div style="max-width:760px; margin:0 auto;">
    ...
    <form action="{{ route('barang.store') }}" method="POST" enctype="multipart/form-data" id="formTambahBarang">
        @csrf

        {{-- Card: Identitas Produk --}}
        <div style="background:#fff;border-radius:16px;padding:28px;box-shadow:0 4px 20px rgba(3,105,161,0.08);border:1px solid #bfdbfe;border-top:4px solid #0ea5e9;margin-bottom:16px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div>
                    ...
                    <input type="text" name="kode" value="{{ old('kode') }}" placeholder="Contoh: BA-009"
                        style="width:100%;padding:10px 14px;border:1.5px solid {{ $errors->has('kode') ? '#ef4444' : '#e2e8f0' }};border-radius:10px;font-size:0.9rem;outline:none;box-sizing:border-box;transition:0.2s;"
                        onfocus="this.style.borderColor='#0369a1';this.style.boxShadow='0 0 0 3px rgba(3,105,161,0.12)'"
                        onblur="this.style.borderColor='#e2e8f0';this.style.boxShadow='none'">
                    @error('kode')<p style="color:#ef4444;font-size:0.78rem;margin:4px 0 0;">{{ $message }}</p>@enderror
                </div>
                <div>
                    ...
                    <input type="text" name="nama" value="{{ old('nama') }}" placeholder="Contoh: Air Mineral Galon"
                        style="width:100%;padding:10px 14px;border:1.5px solid {{ $errors->has('nama') ? '#ef4444' : '#e2e8f0' }};border-radius:10px;font-size:0.9rem;outline:none;box-sizing:border-box;transition:0.2s;"
                        onfocus="this.style.borderColor='#0369a1';this.style.boxShadow='0 0 0 3px rgba(3,105,161,0.12)'"
                        onblur="this.style.borderColor='#e2e8f0';this.style.boxShadow='none'">
                    @error('nama')<p style="color:#ef4444;font-size:0.78rem;margin:4px 0 0;">{{ $message }}</p>@enderror
                </div>
            </div>
            ...
            <div style="margin-top:16px;">
                ...
                <select name="kategori" class="inp-kategori"
                    style="width:100%;padding:10px 14px;border:1.5px solid {{ $errors->has('kategori') ? '#ef4444' : '#e2e8f0' }};border-radius:10px;font-size:0.9rem;outline:none;box-sizing:border-box;background:#f8fafc;cursor:pointer;transition:0.2s;"
                    onfocus="this.style.borderColor='#0369a1';this.style.boxShadow='0 0 0 3px rgba(3,105,161,0.12)'"
                    onblur="this.style.borderColor='#e2e8f0';this.style.boxShadow='none'">
                    <option value="" disabled {{ old('kategori') ? '' : 'selected' }}>-- Pilih Kategori --</option>
                    @foreach(['galon'=>'💧 Galon','botol'=>'🍶 Botol','cup'=>'🥤 Cup'] as $val=>$label)
                    <option value="{{ $val }}" {{ old('kategori')==$val?'selected':'' }}>{{ $label }}</option>
                    @endforeach
                </select>
                @error('kategori')<p style="color:#ef4444;font-size:0.78rem;margin:4px 0 0;">{{ $message }}</p>@enderror
            </div>
        </div>
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:16px;">
                <div>
                    ...
                    <input type="number" name="jumlah" value="{{ old('jumlah',0) }}" min="0"
                        ...
                        onfocus="this.style.borderColor='#0369a1';this.style.boxShadow='0 0 0 3px rgba(3,105,161,0.12)'"
                        onblur="this.style.borderColor='#e2e8f0';this.style.boxShadow='none'">
                    @error('jumlah')<p style="color:#ef4444;font-size:0.78rem;margin:4px 0 0;">{{ $message }}</p>@enderror
                </div>
                <div>
                    ...
                    <input type="text" name="satuan" value="{{ old('satuan') }}" placeholder="pcs, liter, galon"
                        style="width:100%;padding:10px 14px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:0.9rem;outline:none;box-sizing:border-box;"
                        onfocus="this.style.borderColor='#0369a1';this.style.boxShadow='0 0 0 3px rgba(3,105,161,0.12)'"
                        onblur="this.style.borderColor='#e2e8f0';this.style.boxShadow='none'">
                    @error('satuan')<p style="color:#ef4444;font-size:0.78rem;margin:4px 0 0;">{{ $message }}</p>@enderror
                </div>
                <div>
                    ...
                    <input type="number" name="harga" value="{{ old('harga') }}" min="0" placeholder="15000"
                        style="width:100%;padding:10px 14px;border:1.5px solid #e2e8f0;border-radius:10px;font-size:0.9rem;outline:none;box-sizing:border-box;"
                        onfocus="this.style.borderColor='#0369a1';this.style.boxShadow='0 0 0 3px rgba(3,105,161,0.12)'"
                        onblur="this.style.borderColor='#e2e8f0';this.style.boxShadow='none'">
                    @error('harga')<p style="color:#ef4444;font-size:0.78rem;margin:4px 0 0;">{{ $message }}</p>@enderror
                </div>
            </div>
        </div>

        {{-- Card: Info Tambahan + Foto --}}
        <div style="background:#fff;border-radius:16px;padding:28px;box-shadow:0 4px 20px rgba(3,105,161,0.08);border:1px solid #fde68a;border-top:4px solid #f59e0b;margin-bottom:16px;">
            ...
            <div style="display:grid;grid-template-columns:1fr 1fr;gap:16px;">
                <div>
                    ...
                    <input type="text" name="supplier" value="{{ old('supplier') }}" placeholder="Nama supplier"
                        ...
                        onfocus="this.style.borderColor='#0369a1';this.style.boxShadow='0 0 0 3px rgba(3,105,161,0.12)'"
                        onblur="this.style.borderColor='#e2e8f0';this.style.boxShadow='none'">
                    @error('supplier')<p style="color:#ef4444;font-size:0.78rem;margin:4px 0 0;">{{ $message }}</p>@enderror
                </div>
                <div>
                    ...
                    <input type="date" name="tanggal" value="{{ old('tanggal', date('Y-m-d')) }}"
                        ...
                        onfocus="this.style.borderColor='#0369a1';this.style.boxShadow='0 0 0 3px rgba(3,105,161,0.12)'"
                        onblur="this.style.borderColor='#e2e8f0';this.style.boxShadow='none'">
                    @error('tanggal')<p style="color:#ef4444;font-size:0.78rem;margin:4px 0 0;">{{ $message }}</p>@enderror
                </div>
            </div>
            ...
            <div style="margin-top:16px;">
                ...
                {{-- Input file hidden —- PENTING: BUKAN di dalam label dropzone --}}
                ...
                @error('foto')<p style="color:#ef4444;font-size:0.78rem;margin:4px 0 0;">{{ $message }}</p>@enderror
            </div>
        </div>
