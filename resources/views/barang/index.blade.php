...
<div style="max-width:1200px;margin:0 auto;">
    ...
    <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(150px,1fr));gap:12px;margin-bottom:24px;">
        <div style="background:#fff;border-radius:14px;padding:18px 20px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,0.05);">
            <div style="font-size:1.6rem;font-weight:800;color:#0284c7;">{{ $total }}</div>
            <div style="font-size:0.8rem;color:#64748b;margin-top:2px;">Total Produk</div>
        </div>
        <div style="background:#fff;border-radius:14px;padding:18px 20px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,0.05);">
            <div style="font-size:1.6rem;font-weight:800;color:#16a34a;">{{ $tersedia }}</div>
            <div style="font-size:0.8rem;color:#64748b;margin-top:2px;">Stok Tersedia</div>
        </div>
        <div style="background:#fff;border-radius:14px;padding:18px 20px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,0.05);">
            <div style="font-size:1.6rem;font-weight:800;color:#dc2626;">{{ $habis }}</div>
            <div style="font-size:0.8rem;color:#64748b;margin-top:2px;">Stok Habis</div>
        </div>
        <div style="background:#fff;border-radius:14px;padding:18px 20px;border:1px solid #e2e8f0;box-shadow:0 1px 4px rgba(0,0,0,0.05);">
            <div style="font-size:1.6rem;font-weight:800;color:#7c3aed;">{{ $barangs->lastPage() }}</div>
    ...
    <div style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.06);border:1px solid #e2e8f0;">
        ...
        <table style="width:100%;border-collapse:collapse;">
            ...
            <tbody>
                @php $katIcon=['galon'=>'💧','botol'=>'🍶','jerigen'=>'🪣','cup'=>'🥤']; @endphp
                ...
                <tr style="border-bottom:1px solid #f1f5f9;transition:background 0.15s;"
                    ...
                    <td style="padding:14px 16px;">
                        <div style="font-size:0.875rem;font-weight:600;color:#1e293b;">{{ $barang->nama }}</div>
                    </td>
                    <td style="padding:14px 16px;">
                        @php
                        ...
                        @endphp
                        <span style="background:{{ $bg }};color:{{ $fg }};padding:3px 10px;border-radius:99px;font-size:0.78rem;font-weight:600;">
                            {{ $katIcon[$barang->kategori]??'📦' }} {{ ucfirst($barang->kategori) }}
                        </span>
                    </td>
                    <td style="padding:14px 16px;">
                        <span style="font-size:0.875rem;font-weight:700;color:{{ $barang->jumlah > 0 ? '#16a34a' : '#dc2626' }};">
                            {{ $barang->jumlah }}
                        </span>
                        <span style="font-size:0.78rem;color:#94a3b8;"> {{ $barang->satuan }}</span>
                        @if($barang->jumlah <= 0)
                    ...
                    <td style="padding:14px 16px;font-size:0.85rem;color:#475569;">{{ $barang->supplier }}</td>
