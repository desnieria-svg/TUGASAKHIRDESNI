<nav class="site-nav" id="siteNavbar">
    ...
    <img src="<?php echo e(asset('images/logo.png')); ?>" alt="BioAqua Lab" class="site-brand-icon">
            ...
            <ul class="site-nav-menu">
                ...
                <?php if(auth()->guard()->check()): ?>
                    <?php if(auth()->user()->role === 'admin'): ?>
                        <li><a href="<?php echo e(route('barang.index')); ?>" class="nav-link <?php echo e(request()->is('barang*') ? 'active' : ''); ?>">Produk</a></li>
                        <li><a href="<?php echo e(route('admin.pesanan')); ?>" class="nav-link <?php echo e(request()->is('admin/pesanan') ? 'active' : ''); ?>">Pesanan</a></li>
                    <?php else: ?>
                        <li><a href="<?php echo e(url('/#produk')); ?>" class="nav-link <?php echo e(request()->is('produk') ? 'active' : ''); ?>">Produk</a></li>
                        <li><a href="<?php echo e(route('pesanan.riwayat')); ?>" class="nav-link <?php echo e(request()->is('riwayat') || request()->is('pesanan*') ? 'active' : ''); ?>">Pesanan</a></li>
                    <?php endif; ?>
                <?php else: ?>
                    <li><a href="<?php echo e(url('/#produk')); ?>" class="nav-link <?php echo e(request()->is('produk') ? 'active' : ''); ?>">Produk</a></li>
                <?php endif; ?>
                ...
                <div class="user-menu-wrap">
                    ...
                    <div class="user-dropdown" id="userDropdown">
                        ...
                        <a href="<?php echo e(route('profile.edit')); ?>" class="dropdown-item">
                            <svg viewBox="0 0 24 24" fill="none" width="16" height="16" xmlns="http://www.w3.org/2000/svg"><circle cx="12" cy="8" r="4" stroke="currentColor" stroke-width="1.7"/><path d="M4 20c0-4 3.6-7 8-7s8 3 8 7" stroke="currentColor" stroke-width="1.7" stroke-linecap="round" stroke-linejoin="round"/></svg>
                            Profil
                        </a>
...
.nav-dropdown-menu a:hover { background: #f0f9ff; color: #0369a1; }
.nav-dropdown-hint { font-size: 0.68rem; color: #94a3b8; padding: 6px 12px 2px; white-space: normal; max-width: 200px; line-height: 1.4; }
.dark-mode .nav-dropdown-menu { background: #1e293b !important; border-color: #334155 !important; }
.dark-mode .nav-dropdown-menu a { color: #cbd5e1 !important; }
.dark-mode .nav-dropdown-menu a:hover { background: #1e3a5f !important; color: #38bdf8 !important; }
.dark-mode .nav-dropdown-hint { color: #64748b !important; }
...
.dark-mode .qty-num { color: #f1f5f9 !important; }
</style>
...
window.tambahKeranjang = function(btn, nama, harga, icon, id, kategori, foto, stok) {
    stok = (stok === undefined || stok === null || stok === '') ? Infinity : parseInt(stok, 10);
    if (!window._cart[nama]) window._cart[nama] = {id: id||0, nama, harga: harga||0, icon: icon||'💧', kategori: kategori||'', foto: foto||'', qty: 0, stok: stok};
    window._cart[nama].stok = stok;
    if (window._cart[nama].qty >= window._cart[nama].stok) {
        if (typeof tampilToast === 'function') tampilToast('⚠️ Stok ' + nama + ' tidak cukup!');
        else alert('Stok ' + nama + ' tidak cukup!');
        return;
    }
...
window._changeQty = function(nama, delta) {
    ...
    const item = window._cart[nama];
    const stok = (item.stok === undefined || item.stok === null) ? Infinity : item.stok;
    if (delta > 0 && item.qty >= stok) {
        if (typeof tampilToast === 'function') tampilToast('⚠️ Stok ' + nama + ' tidak cukup!');
        else alert('Stok ' + nama + ' tidak cukup!');
        return;
    }
...
<?php /**PATH C:\xampp\htdocs\BioAquaLab\BioAquaLab\resources\views/partials/navbar.blade.php ENDPATH**/ ?>